...
public:
    ...
    {
        switch(s->current_camera_type)
        {
        case BASE_CAMERA:
            camera=&(s->camera);
            break;
        case LEN_BASE_CAMERA:
            camera=&(s->lb_camera);
            break;
        }
        ClampBounds();
    ...
    virtual Color3f Li(const Ray& ray, const Scene& scene, std::shared_ptr<Sampler> sampler, int depth, Color3f beta) const = 0;
...
protected:
    ...
    Camera const * camera;			// A pointer to the Camera instance stored in Scene.
