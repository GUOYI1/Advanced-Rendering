...
void MyGL::RenderScene()
{
    ...
    //When both BVH tree and KD tree are checked, only BVH tree will be constructed.
    if(!makeBVH)
    {
        if(makeKD && !scene.kd)
        {
            scene.kd=new KdTreeAccel(scene.primitives.toVector().toStdVector(), maxBVHPrims);
        }
        else if(makeKD && scene.kd)
        {
            scene.clearKD();
            scene.kd=new KdTreeAccel(scene.primitives.toVector().toStdVector(), maxBVHPrims);
        }
    ...
    for(int X = 0; X < nTiles.x; X++)
    {
        for(int Y = 0; Y < nTiles.y; Y++)
        {
            ...
            scene.lb_camera.sampler=sampler->Clone(seed);
...
void MyGL::slot_SetCameraType(int t)
{
    switch(t)
    {
    case 0:
        scene.current_camera_type = BASE_CAMERA;
        break;
    case 1:
        scene.current_camera_type = LEN_BASE_CAMERA;
        break;
    }
}
