...
Scene::Scene() : bvh(nullptr),kd(nullptr),current_camera_type(BASE_CAMERA)
{}
...
{
    camera = Camera(c);
    lb_camera=Len_Base_Camera(camera,5,30);
    camera.create();
    film.SetDimensions(c.width, c.height);
}

void Scene::SetLBCamera(Float lensRadius, Float focalDistance)
{
    lb_camera.lensRadius=lensRadius;
